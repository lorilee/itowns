...
function drawNextLayer(storages) {
    ...
    // see https://gamedev.stackexchange.com/a/95696 for a details
    let sum = 0;
    let selected;
    let max;
...
Scheduler.prototype.execute = function execute(command) {
    ...
    if (q.counters.executing < this.maxCommandsPerHost) {
        // We use a setTimeout to defer processing
        // (why setTimeout and not Promise? see tasks vs microtasks priorities)
        window.setTimeout(() => {
            if (q.counters.executing < this.maxCommandsPerHost) {
